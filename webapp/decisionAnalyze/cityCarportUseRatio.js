...
myapp.controller("cityCarportUseRatio", function ($scope, $http, $state) {
     ...
     *  初始化查询条件，获取行政区域编码
     * </p>
     * @param $scope.cityRatioArr：停车场集合<br>
     * @param pkPosAreaCode：停车场区域代码<br>
     * @param pageNum：当前页码<br>
     * @param pageSize：每页总条数<br>
    ...
    $scope.pkSpcheck = function () {
        $scope.cityRatioArr = {}
        $scope.cityRatioArr.cityCode = $("select[name='sheng']").val();
        $scope.cityRatioArr.status = -1;
        $scope.cityRatioArr.pageNum = "1";
        $scope.cityRatioArr.pageSize = "10000";

        $http({
            url: "/eleting-web/area/getAreaPageInfoByCondition",
            method: "get",
            params: $scope.cityRatioArr,
        }).success(function (data) {
            if (data.datum.list.length < 1) {
                alert("未查询到结果或者未点击查询按钮!!!!")
            }
            $scope.cityRatioArr.cityRatioName = data.datum.list;
        }).error(function () {
...
//    改变区域
     ...
     *  查询车位收费情况
     * </p>
     * @param $scope.text：选择的停车场集合<br>
     * @param $scope.areaRadiocharqeLinearr：查询时间区间<br>
     * 创建者：肖烈 创建时间: 2017-04-19<br>
     *  @author 肖烈
     * @version 1.0.0.0
     */
    ...
    $scope.submitSplaceId = function () {
        text = [];
        $("input[name=cityRadioChecked]").each(function () {
     ...
     *  自动生成日期
     * </p>
     * @param dreDate：日期集合<br>
     * @param dreDateLast：重置日期集合<br>
     * 创建者：肖烈 创建时间: 2017-04-19<br>
     *  @author 肖烈
     * @version 1.0.0.0
     */
